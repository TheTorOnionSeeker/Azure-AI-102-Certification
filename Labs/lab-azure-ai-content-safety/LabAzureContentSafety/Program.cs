...
string userText = "Se encontró a un hombre de 51 años muerto en su coche. Había manchas de sangre en el salpicadero y el parabrisas." +
    "En la autopsia, se encontró un corte profundo, oblicuo y largo en la parte frontal del cuello. Resulta que ha muerto por suicidio.";

var request = new AnalyzeTextOptions(userText);

Response<AnalyzeTextResult> response;
Response<AnalyzeImageResult> responseImage;

response = client.AnalyzeText(request);
responseImage = client.AnalyzeImage(new Uri("https://cdn.apollohospitals.com/health-library-prod/2021/09/Self-injurycutting.png"));
...
Console.WriteLine("\nAnalyse image succeeded:");
Console.WriteLine("Hate severity: {0}", responseImage.Value.CategoriesAnalysis.FirstOrDefault(a => a.Category == ImageCategory.Hate)?.Severity ?? 0);
Console.WriteLine("SelfHarm severity: {0}", responseImage.Value.CategoriesAnalysis.FirstOrDefault(a => a.Category == ImageCategory.SelfHarm)?.Severity ?? 0);
Console.WriteLine("Sexual severity: {0}", responseImage.Value.CategoriesAnalysis.FirstOrDefault(a => a.Category == ImageCategory.Sexual)?.Severity ?? 0);
Console.WriteLine("Violence severity: {0}", responseImage.Value.CategoriesAnalysis.FirstOrDefault(a => a.Category == ImageCategory.Violence)?.Severity ?? 0);
...
namespace ContentSafetySampleCode
{
    ...
    public class Program
    {
        static async Task Main(string[] args)
        {
            ...
            string endpoint = "https://lab-content-safety-guilledev.cognitiveservices.azure.com/";
            string subscriptionKey = "BGjyydbr7Vt9uLdtoq5D6Muonwf3ZRgNSavsKenfXfCECv44gYq1JQQJ99BBACYeBjFXJ3w3AAAHACOGbDyU";
            ...
            string content = "Ya dejalo, idiota!";
            ...
            Dictionary<Category, int> rejectThresholds = new Dictionary<Category, int> {
                { Category.Hate, 2 }, { Category.SelfHarm, 2 }, { Category.Sexual, 2 }, { Category.Violence, 2 }
            };
